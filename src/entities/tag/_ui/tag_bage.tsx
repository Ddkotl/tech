...
export function TagBage({ slug, title }: PartialTag): JSX.Element {
  return (
    <Badge
      key={slug}
      className="h-6 bg-fio transition-colors duration-300 hover:scale-95 hover:bg-slate-950 text-slate-100 text-xs font-medium px-2.5 py-1.5 z-40"
    >
...
export function TagBageSkeleton() {
  return <Skeleton className="h-7 w-12 sm:w-16 rounded-md" />;
}
